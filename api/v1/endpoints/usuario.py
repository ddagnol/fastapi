...
#post / Signup, ou seja, criar a conta
...
async def post_usuario(usuario: UsuarioSchemaCreate,db: AsyncSession= Depends(get_session)):
    ...
    async with db as session:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE,detail='Já existe um usuario cadastrado com este email')
...
async def get_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if usuario:
            ...
            raise HTTPException(detail='Usuário nao encontrado.',
